...
func NewJSTimelineRuntimeWithOptions(opts JSTimelineRuntimeOptions) (*JSTimelineRuntime, error) {
	...
	builder := gojengine.NewRuntimeFactoryBuilder(builderOpts...).WithModules(
		gojengine.NativeModuleRegistrar{
	...
	ownedRuntime, err := factory.NewRuntime(gojengine.WithStartupContext(context.Background()), gojengine.WithLifetimeContext(context.Background()))
	if err != nil {
